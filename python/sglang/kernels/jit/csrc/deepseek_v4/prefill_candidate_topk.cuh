...
template <bool kPDL>
__global__ __launch_bounds__(impl::TopKConfig::kBlockSize, impl::TopKConfig::kOccupancy)
void select(const __grid_constant__ Params params) {
  ...
  if (length <= params.topk) {
    ...
    for (uint32_t i = threadIdx.x; i < params.topk; i += blockDim.x) {
      const bool valid = i < length && candidates[i / params.block_size]
                         && scores[i] > -impl::infinity_value();
  ...
  for (uint32_t i = threadIdx.x; i < params.topk; i += blockDim.x) {
    ...
    const bool valid = index >= 0 && index < length
                       && candidates[index / params.block_size]
...
struct Kernel {
  static void run(
    ...
    RuntimeCheck(candidates.dtype().code == kDLBool && candidates.dtype().bits == 8 && candidates.dtype().lanes == 1, "candidate mask must have dtype bool");
